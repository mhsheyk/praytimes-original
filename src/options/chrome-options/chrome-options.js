...
(function() {
  ...
  function setup() {
    ...
    var extensionName = chromeOptions.opts.title || manifest.name || 'chrome';
    ...
    if (chromeOptions.opts.title !== false && !urlParams.hideTitle) {
      document.body.classList.add('show-title');
    }

    if (chromeOptions.opts.about !== false &&
       (chromeOptions.opts.about || manifest.description) &&
        !urlParams.hideAbout) {
      document.body.classList.add('show-about');
      let $about = document.querySelector('#about .content > p');
      if (chromeOptions.opts.about) {
        $($about).empty().append($('#about-content'));
      } else {
    ...
    if (chromeOptions.opts.autoSave) {
   ...
   */
  chromeOptions.addTab = function(name, desc, options) {
    setup();
    if (!options) {
      options = desc;
      desc = null;
   ...
   * @param {Array.<Object>} options
   */
  chromeOptions.set = function(desc, options) {
    urlParams.hideSidebar = true;
    urlParams.hideTabTitle = true;
    chromeOptions.addTab('', desc, options);
  };
  ...
  function addOption(key, values, value, save, option, top) {
    ...
    if (value === undefined && option.default != null) {
      ...
      if (chromeOptions.opts.saveDefaults) {
        // console.log('Save Default: ', key, value, save);
        save(value);
      }
    }
    ...
    switch (option.type) {
      case 'checkbox':
        $option = chromeOptions.base.checkbox(value, save, option, key);
        break;
      case 'object':
        $option = chromeOptions.base.object(value, save, option, key);
        break;
      case 'list':
        $option = chromeOptions.base.list(value, save, option, key);
        break;
      case 'column':
        $option = chromeOptions.base.column(values, save, option, key, top);
        break;
      case 'row':
        $option = chromeOptions.base.row(values, save, option, key, top);
        break;
      case 'h3':
        $option = addH3(option);
        break;
      ...
      default:
        if (!option.type) {
          $option = chromeOptions.base.checkbox(value, save, option, key);
        } else if (chromeOptions.fields[option.type]) {
          $option = chromeOptions.addLabelNField(value, save, option);
        } else if ((r = /(\w+)-list/.exec(option.type))) {
          $option = chromeOptions.base
  ...
  chromeOptions.base.checkbox = function(value, save, option, key) {
    var $label = h('label');
    ...
    var $checkbox = chromeOptions.fields.checkbox(checked, function(checked) {
      if (hasOptions) {
  ...
  chromeOptions.base.checkboxNField = function(value, save, option, type) {
    if (value == null || typeof value !== 'object') {
      value = {};
    }
    var mustSave = false;
    ...
    if (mustSave && chromeOptions.opts.saveDefaults) {
      save(value);
    }

    if (!chromeOptions.fields[type]) {
    ...
    $box
      .append(chromeOptions.fields.checkbox(value.enabled, function(checked) {
        value.enabled = checked;
        save(value);
      }, option));

    $container.append(chromeOptions.addField(value.value, function(newValue) {
  ...
  chromeOptions.base.object = function(value, save, option, key) {
  ...
  chromeOptions.addLabelNField = function(value, save, option) {
    var $container = h('.suboption', h('label', option.desc || ''));
    var $field = chromeOptions.addField(value, save, option);
    $container.append(h('.field-container', $field));
    $container.classList.add(option.singleline ? 'singleline' : 'multiline');
    return $container;
  };

  chromeOptions.base.list = function(list, save, options, key) {
  ...
  function addListRow($table, values, fields, fieldsMap, save, remove,
    ...
    var fieldUpdates = fields.map(function(field) {
      ...
      if (chromeOptions.fields[field.type]) {
        $field = chromeOptions.addField(fieldValue, saveField, field);
      } else if (field.type === 'column') {
        $field = chromeOptions.base.column(values, save, field, key);
      } else if (field.type === 'row') {
        $field = chromeOptions.base.row(values, save, field, key);
      } else {
  ...
  chromeOptions.base.singleFieldList = function(value, save, options, type) {
    options.fields = [{ type: type, name: options.name }];
    return chromeOptions.base.list(value, save, options);
  };

  chromeOptions.base.column = function(values, save, option, key, top) {
    delete option.name;
    var $container;
    if (top) {
  ...
  chromeOptions.base.row = function(values, save, option, key, top) {
    var $container =  chromeOptions.base.column(values, save, option, key, top);
    $container.classList.add('row');
    return $container;
  };

  chromeOptions.addField = function(value, save, option, type) {
    var fn = chromeOptions.fields[type || option.type];
    if (!fn) { return; }
    var lastTimeStamp;
...
chromeOptions.fields = {};

chromeOptions.fields.checkbox = function(value, save) {
...
chromeOptions.fields.text = function(value, save) {
...
chromeOptions.fields.color = function(value, save, option) {
  var first = true;
  ...
  var $field = chromeOptions.fields.text(value, function() {
    if ($alpha) {
...
chromeOptions.fields.url = function(value, save, option) {
  var $field = chromeOptions.fields.text(value, save, option);
  $field.setAttribute('type', 'url');
  return $field;
};

chromeOptions.fields.select = function(value, save, option) {
  var valueMap = {};
  var $select = h('select', {
    onchange: function(e) {
...
chromeOptions.fields.radio = function(value, save, option) {
